...
use App\Factory\CompanyActivityFactory;
...
class CompanyFixtures extends Fixture implements DependentFixtureInterface
{
    public function load(ObjectManager $manager)
    {
        ...
        for($i = 1; $i <= 3; $i++) {
            $company->addActivity($this->getReference(CompanyActivityFixtures::REFERENCE.rand(0, 11)));           
        }
        for($i = 0; $i <= 3; $i++) {
        ...
        [
            'companyImages' => CompanyImageFactory::createMany(rand(5, 5)),
            'social_Links' => SocialLinkFactory::createMany(3, 
            [
                'social_network' => $this->getReference(SocialNetworkFixtures::REFERENCE.rand(0,3))
            ]), 
            'activities' => CompanyActivityFactory::randomSet(rand(1, 4))
        ]);
    }
